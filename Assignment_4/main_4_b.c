...
//	out=read(fd,&rdbf[0],1);
	...
	buf[0]=0x6B;buf[1]=0x00;   //for writing 0b000000000 to power management register to disable sleep mode if in sleep mode
		printf("i2c_fd sorry %d %d ",i2c_fd,out);
...
a[0]=((float)((H_byte<<8)+L_byte))/(16384);
//for acceleration y
...
status=read(fd,&H_byte,1);
a[1]=((float)((H_byte<<8)+L_byte))/(16384);
//for acceleration z
...
status=read(fd,&H_byte,1);
a[2]=((float)((H_byte<<8)+L_byte))/(16384);
//for gyro x
...
g[0]=((float)250*((H_byte<<8)+L_byte))/(32768);
//for gyro y
...
g[1]=((float)250*((H_byte<<8)+L_byte))/(32768);
//for gyro z
...
g[2]=((float)250*((H_byte<<8)+L_byte))/(32768);
//for temp 
...
status=write(fd,&add_h,1);
status=read(fd,&H_byte,1);
t=((float)((H_byte<<8)+L_byte))/340 + 36.53;
}
close(fd_edg);
close(gpio_fd);
return status;
