...
//angle[]=kalmanCalculate((atan2f(a[0]-acc_m[0],a[2]-acc_m[2])* 180 / MY_PI),g[0]);
	...
	angle[2]=(angle[2]+((g[2]-ang_vel_m[2])*(dt)));//+ 0.02*(atan2f((float)a[1], (float)a[0]) * 180 / MY_PI);
gp[2]=g[2];
...
//	scanf("%f",&a[0]);scanf("%f",&a[1]);scanf("%f",&a[2]);
