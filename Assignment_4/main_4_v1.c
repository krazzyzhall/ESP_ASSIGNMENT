...
//	prev=rdtsc();
	...
	a[2]=(float)((raw[4]<<8) + raw[5])*(ac_val) - 5.0;  //there is an error in my MPU
...
float roll;
float pitch,ap[3];
float get_angle_pos(float *g,float *a){
static float d=0,acc=0;
...
//	a[0]=a[0];
	...
	printf(" xd : %f ,a : %f ,u %f,acc diff :%f ,ap :%f\n",d*100,acc*100,u[0]*100,a[0]-ap[0],100*ap[0]);
	ap[0]=a[0];
}
...
#ifdef RESTRICT_PITCH // Eq. 25 and 26  for RXYZ reference frame 
  roll  = atan2f(a[1], a[2]);
  pitch = atan(-a[0] / sqrt(a[1] * a[1] + a[2] * a[2]));
#else // Eq. 28 and 29
  roll  = atan(a[1] / sqrt(a[0] * a[0] + a[2] * a[2]));
  pitch = atan2f(-a[0], a[2]);
#endif
...
//angle[]=kalmanCalculate((atan2f(a[0]-acc_m[0],a[2]-acc_m[2])* 180 / MY_PI),g[0]);
	if(abs(g[2]-gp[2])>2*ang_vel_s[2])
	angle[2]=0.98*(angle[2]+((g[2]-ang_vel_m[2])*(dt)))+ 0.02*(atan2f((float)a[1], (float)a[0]) * 180 / MY_PI);
//printf("x angle : %f ____ %f ",angle[0],roll* 180 / MY_PI);
//printf("y angle : %f ____ %f \n",angle[1],pitch* 180 / MY_PI);
return status;
