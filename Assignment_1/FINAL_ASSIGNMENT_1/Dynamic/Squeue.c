...
ipc_msg *temp;
        int check=0,retval,cread,cwrite;
        ...
        if((Squeue_devp->current_write_pointer-Squeue_devp->current_read_pointer>0) &&
           (Squeue_devp->current_write_pointer-Squeue_devp->current_read_pointer<=10) ){
            if(cwrite-cread!=0){
                   ...
                   Squeue_devp->msg[cwrite]->enqueue_time=rdtsc();
...
if(Squeue_devp->msg[cread]==NULL){
            mutex_unlock(&my_mutex);
             return -100;}
        Squeue_devp->msg[cread]->accumulated_queing_time+=rdtsc()-Squeue_devp->msg[cread]->enqueue_time;
