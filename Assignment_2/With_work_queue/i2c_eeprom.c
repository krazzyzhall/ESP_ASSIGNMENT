...
typedef struct {struct work_struct my_work; struct i2c_client *client;struct payload *buf; u16 count;u16 add; int ret;} my_work_t;
my_work_t *work,*work2; 

static void write_to_i2c(struct work_struct *work2){
...
static void read_from_i2c(struct work_struct *work2){
my_work_t *work=(my_work_t *)work2;
int ret=-1; 
u16 add=work->add;
while(ret<0)   
...
static ssize_t
i2ceeprom_read (struct file *file, char __user * buf, size_t count,loff_t * offset)
{
char *tmp;
int ret=-1;
...
ready=-1;
 ...
 if(work){
   ...
   work->count=64;
   work->buf=(struct payload *)kmalloc(sizeof(struct payload),GFP_KERNEL);
...
u16 get_pg_no(struct i2c_clnt *clnt){
  u16 addr=clnt->mesg->addr;
return addr;
}
int set_pg_no(u16 pg_no,struct i2c_clnt *client){
    client->mesg->addr=pg_no;
    return pg_no;
}
int status_flash(struct i2c_client *client){
return i2c_master_send(client,"a",1);
}
int erase_flash(struct i2c_client *client,u16 pages){ 
   char mesg[66];
   struct mywrite{ 
   u16 add;
   char mesg[64];
...
pack->client =(struct i2c_client*)kzalloc(sizeof(struct i2c_client),GFP_KERNEL);
if(!pack->client) {
	i2c_put_adapter(adap);
         return -ENOMEM;
}
...
static int i2cdev_attach_adapter(struct device *dev,void*dummy){
 ...
 i2c_dev->dev = device_create(i2c_dev_class ,NULL,MKDEV(I2C_MAJOR,adap->nr),NULL,"i2c-flash");
