...
static int sequence_run(void *work){
		...
		//struct spidev_data *spidev=kmalloc(sizeof(struct spidev_data))
                 //memcpy(spidev,((struct kthreadp*)work)->spidev,sizeof(struct spidev_data));
		 int i=0,j;
		char temp[2];                 
		printk("sequence_run  thread is now live : %d  \n",spidev->seq.lseq);
/*while(){
                             msleep(1000);
if (kill==1){kill=0;break;}
 set_current_state(TASK_INTERRUPTIBLE);  ///to make the thread interruptible
}*/		
 printk("before wake_up_process in seq_run %d : %d : %d : %d \n ",spidev->seq.seq[0],spidev->seq.seq[1],spidev->seq.seq[2],spidev->seq.seq[3]);                 
...
set_current_state(TASK_INTERRUPTIBLE);  ///to make the thread interruptible
                             ...
                             msleep(1000);
 printk("after wake_up_process under_run seq %d : %d : %d : %d \n ",spidev->seq.seq[0],spidev->seq.seq[1],spidev->seq.seq[2],spidev->seq.seq[3]);                 
return status;
}
...
struct spidev_data      *spidev;
        ...
        spidev = filp->private_data;
    		...
    		printk("spidev write : in else part %x %x  %d\n",spidev->buffer[0],spidev->buffer[1],count);                         
	        status = spidev_sync_write(spidev, count); 
                 mutex_unlock(&spidev->buf_lock);                            
                 }
printk("at the end of write function \n");
         return status;
}
...
int status=0;
struct kthreadp *work=(struct kthreadp *)kmalloc(sizeof(struct kthreadp),GFP_KERNEL);
spidev = filp->private_data;
switch(cmd){
case ISPATTERN:
...
case INS_SEQ:
   ...
   work->count=0;
   work->status=status;
     //  printk("before kthread create %d : %d : %d : %d \n ",spidev->seq.seq[0],spidev->seq.seq[1],spidev->seq.seq[2],spidev->seq.seq[3]);
thread1 = kthread_create(sequence_run,(void *)work,"write_thread");
	...
	status= memcpy(&(spidev->seq),(struct my_seq_pat *)arg,sizeof(struct my_seq_pat));	        
       ...
       printk("before wake_up_process %d : %d : %d : %d \n ",spidev->seq.seq[0],spidev->seq.seq[1],spidev->seq.seq[2],spidev->seq.seq[3]);
        printk(KERN_INFO "in if  thread created and is ready to wake up\n");
...
printk("just enered close  \n"); 
		...
		if(kill!=1){
                 ...
                 spin_unlock_irq(&spidev->spi_lock);
 printk("at the end of if close function \n"); 
                 ///if (dofree)
                  //       kfree(spidev);
//kill=1;
 status=kthread_stop(thread1);
